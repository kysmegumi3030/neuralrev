...
def excite(n: int, at: int, kind: str = "burst") -> np.ndarray:
    """激励：8192 点 Hann 窗白噪猝发。

    用白噪而不是正弦：逐 bin 口径要求**每个 bin 都有能量**，单频只能验一个 bin。
    用猝发而不是连续噪声：避免稳态下的相位纠缠。

    ## 长度 8192 是**按地板量出来的**，不是随手取的（§14.14.9）

    原先取 2048，当时没量它的地板 —— 那是个错误。实测
    （`ref_delay_excite_floor.py`，参考**自比**、激励偏移 1 样点、
    同一个 smoothed_spectrum_err_db(floor_db=−40)）：

    | 猝发长度 | 1100ms 地板 | 0.9 地板 | 0.65 地板 |
    |---|---|---|---|
    | 2048  | **3.44** | 1.59 | 0.18 |
    | 3072  | 1.77 | 1.51 | 0.22 |
    | 4800  | 1.10 | 1.57 | 0.12 |
    | 6144  | 1.18 | 1.22 | 0.09 |
    | **8192** | **1.11** | **0.74** | **0.23** |
    | 12000 | 0.94 | 0.66 | 0.11 |

    2048 点在 1100 ms 档上地板 **3.44 dB —— 已经超过 3 dB 判据本身**。
    即那个激励在长档上**没有分辨力**：参考与它自己比就判不过，
    它报的 4.62 dB 里哪部分是失配、哪部分是本底，无法区分。

    机制（可预测，且实测按它走）：猝发长 L 的谱相关宽度 ≈ SR/L，
    而平滑带（oct_frac=1/12 是半宽 ⇒ 合计 1/6 oct）在 12 kHz 宽约 1400 Hz。
    L 越小 ⇒ 带内**独立**样本越少 ⇒ 带内 RMS 估计方差越大 ⇒ 地板抬高。
    Hann 又把等效支撑压到约 L/2，所以短猝发 + Hann 是最差组合。
    地板随 L 单调下降到约 0.6–1.1 后**转平**（被别的因素接管），
    这条单调性就是这套解释的检验 —— 它通过了。

    ## 为什么换激励是合法的，而不是「挑一个好看的数」

    换的依据是**地板**：参考与自己比，候选完全不参与，它是仪器的属性。
    用户定的口径是「65536 点 FFT、每 bin ≤3 dB」，**没有**规定猝发长度 ——
    长度是实现细节。而且「读数 − 地板」在所有长度、两种窗形下都稳定在
    1.0–1.7 dB，即候选的真实贡献是个常数，换仪器并没有改变被测量的东西，
    只是让它露出来。

    上界 L ≤ 12736：分析窗是 y[at : at+65536]，最长档 D=52800，
    猝发尾巴与该档回声都必须在窗内（8192 + 52800 = 60992 ✓）。
    """
